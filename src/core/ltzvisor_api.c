...
void ltzvisor_kickoff(void){
	...
	/** Exit from Monitor mode without arguments.*/
	LTZVISOR_MON_EXIT()

	/** Sfreertos entry point */
	main_secure();
...
//	#endif
	...
	memcpy((uint32_tee *)g->gce_bin_load,(uint32_tee *)g->gce_bin_start,(g->gce_bin_end - g->gce_bin_start));
	if(g->gce_trd_init) {
		memcpy((uint32_tee *)g->gce_trd_load,(uint32_tee *)g->gce_trd_start,(g->gce_trd_end - g->gce_trd_start));
	}
	printk("      * NS_Guest load - OK  \n\t");

	/* Get NS_Guest ready to run */
	cp15_restore(&NS_Guest.core.vcpu_regs_cp15);
	set_guest_context( (uint32_tee)&NS_Guest);
