...
/** For printk */
#define serial_puts(str_buffer) uart_puts(1,(const int8_tee *)str_buffer)

/** Zynq UART register structure */
typedef struct{
	/* UART Control register */
	volatile uint32_tee control;
	/* UART Mode Register */       
	volatile uint32_tee mode;
	/* UART Interrupt Enable Register */          
	volatile uint32_tee isr_en;
	/* UART Interrupt Disable Register */     
	volatile uint32_tee isr_dis;
	/* UART Interrupt Mask Register */     
	volatile uint32_tee isr_mask;
	/* UART Channel Interrupt Status Register */     
	volatile uint32_tee isr_status;
	/* UART Baud Rate Generator Register */  
	volatile uint32_tee br_gen;
	/* UART Receiver Time out Register */       
	volatile uint32_tee rx_timeout;
	/* UART Receiver FIFO Trigger Level Register */    
	volatile uint32_tee rx_fifo_trig;
	/* UART Modem Control Register */  
	volatile uint32_tee modem_ctrl;
	/* UART Modem Status Register */    
	volatile uint32_tee modem_status;
	/* UART Channel Status Register */ 
	volatile uint32_tee ch_status;
	/* UART Transmit and Receive FIFO */    
	volatile uint32_tee tx_rx_fifo;
	/* UART Baud Rate Divider Register */   
	volatile uint32_tee br_div;
	/* UART Flow Control Delay Register */       
	volatile uint32_tee flow_ctrl_dl;
		/* Reserved: 2(0x2) bytes */  
	const uint32_tee reserved[2];
	/* UART Transmitter FIFO Trigger Level Register */    	
	volatile uint32_tee tx_fifo_trig;
} Uart_Zynq;
 ...
 */
uint32_tee uart_init(uint8_tee uart_id);
 ...
 * @retval	
 */
void uart_enable(uint8_tee uart_id);
 ...
 * @retval	
 */
void uart_disable(uint8_tee uart_id);
 ...
 * @retval	TRUE in sucess; FALSE in case error	
 */
uint32_tee uart_set_baud_rate(uint8_tee uart_id, uint32_tee baud_rate);
 ...
 */
uint32_tee uart_getc(uint8_tee uart_id);
 ...
 * @retval		
 */
void uart_putc(uint8_tee uart_id,int8_tee c);
 ...
 * @retval		
 */
void uart_puts(uint8_tee uart_id,const int8_tee *s);
